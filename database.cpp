...
void Database::changeUserStatusBun(string userName, bool status)
{
    for(uint i=0; i<_users.size(); i++) {
        if(_users.at(i).getuserName() == userName) {
            _users.at(i).setBun(status);
        }
    }
}
...
void Database::deleteUser(string userName)
{
    for(uint i=0; i<_users.size(); i++) {
        if (_users.at(i).getuserName() == userName) {
            _users.erase(_users.begin() + static_cast<int>(i));
            _usersMapByName.erase(_usersMapByName.find(userName));
            }
        }
}
